...
if [[ $BUILD_SYS == gradle ]]; then
  ...
  if [[ "$(uname)" == "Linux" ]]; then
    CMD[graalvm]="./gradlew -Dorg.gradle.jvmargs=\"${GRAALVM_GRADLE_ARGS}\" --build-cache --parallel clean nativeCompile --pgo-instrument --build-args=\"--gc=G1 --XX:MaxRAMPercentage=90.0\""
  else
