...
if [[ $BUILD_SYS == gradle ]]; then
  ...
  if [[ "$(uname)" == "Linux" ]]; then
    CMD[graalvm]="SPRING_PROFILES_ACTIVE=postgres ./gradlew -Dorg.gradle.jvmargs=\"${GRAALVM_GRADLE_ARGS}\" --build-cache --parallel clean nativeCompile --pgo-instrument --build-args='--gc=G1' --build-args='-R:MaxHeapSize=128m' --build-args='--parallelism=4' --jvm-args-native=\"-Xmx128m\""
  else
    CMD[graalvm]="SPRING_PROFILES_ACTIVE=postgres ./gradlew -Dorg.gradle.jvmargs=\"${GRAALVM_GRADLE_ARGS}\" --build-cache --parallel clean nativeCompile --pgo-instrument --build-args='--parallelism=4' --build-args='-R:MaxHeapSize=128m' --jvm-args-native=\"-Xmx128m\""
  fi
...
for label in "${REQUESTED[@]}"; do
  ...
  if [[ "$label" == "graalvm" ]]; then
    ...
    if [[ "$(uname)" == "Linux" ]]; then
      rebuild_cmd="SPRING_PROFILES_ACTIVE=postgres ./gradlew -Dorg.gradle.jvmargs=\"${GRAALVM_GRADLE_ARGS}\" --build-cache --parallel clean nativeCompile --build-args='--gc=G1' --build-args='-R:MaxHeapSize=128m' --build-args='--parallelism=4' --jvm-args-native=\"-Xmx128m\""
    else
      rebuild_cmd="SPRING_PROFILES_ACTIVE=postgres ./gradlew -Dorg.gradle.jvmargs=\"${GRAALVM_GRADLE_ARGS}\" --build-cache --parallel clean nativeCompile --build-args='--parallelism=4' --build-args='-R:MaxHeapSize=128m' --jvm-args-native=\"-Xmx128m\""
    fi
    echo "-> $rebuild_cmd"
    echo
