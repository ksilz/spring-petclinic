...
if [[ "$LABEL" == "graalvm" ]]; then
  ...
  # Use absolute JAR path with -jar for CRaC to avoid ClassNotFoundException during restore
  ABSOLUTE_JAR_PATH=$(realpath "$JAR_PATH")
  APP_CMD="java -Xms512m -Xmx1g -Dspring.aot.enabled=false -XX:CRaCRestoreFrom=petclinic-crac -XX:CRaCEngine=warp -jar $ABSOLUTE_JAR_PATH --spring.profiles.active=postgres --spring.datasource.hikari.allow-pool-suspension=true"
  TRAIN_CMD="java -XX:+UseG1GC -Dspring.aot.enabled=false -XX:CRaCCheckpointTo=petclinic-crac -XX:CRaCEngine=warp -jar $ABSOLUTE_JAR_PATH --spring.profiles.active=postgres --spring.datasource.hikari.allow-pool-suspension=true"
else
